...
function iotUserDataDirective(){
	return {
	    restrict: 'E', // C: class, E: element, M: comments, A: attributes
	    replace: true, // replaces original content with template
	    templateUrl: "directives/iot-user-data.html",
	    controller: iotUserDataController
	  };
}

function loadIoTUserData($scope, $http){
	$scope.userInfoLoading = true;
	$scope.$parent.activeAlert = false;
	
	$http.get('/data/aggregated/iotdata/' + $scope.context.currentUser ).success(function(data) {
		$scope.datasource = data;
		$scope.$parent.activeAlert = data.activeAlert;
	}).finally( function(){
		$scope.userInfoLoading = false;
	});
}

function iotUserDataController($scope, $http){
	loadIoTUserData($scope, $http);
...
// directives
app.directive('iotUserData', [iotUserDataDirective]);
