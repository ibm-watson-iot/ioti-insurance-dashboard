...
function mainController($scope, $http, $window, aggregator) {
	...
	$http.get('/data/myUser'). success(function(data) {
		console.log(data.fullname);
		$scope.myUser = data.fullname;
	});

	$scope.logout = function() {
		$http.get('/data/logout')
	...
	$scope.$watch('context.currentLocation', function(newValue, oldValue) {
		if (newValue !== oldValue) {
			getPageData($scope, $http, $scope.selectedDate.toISOString(), newValue);
		}
	});

    $scope.$watch('context.currentUser', function(newValue, oldValue) {
        if (newValue) {
          getPageData($scope, $http, $scope.context.selectedDate, $scope.currentLocation);
        }
    });
    */
}
