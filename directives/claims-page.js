...
function claimsPageDirective(){
	return {
	    restrict: 'E', // C: class, E: element, M: comments, A: attributes
	    replace: true, // replaces original content with template
	    templateUrl: "directives/claims-page.html",
	    controller: claimsPageController
	  };
}

function claimsPageController($scope, $http, aggregator, regionData) {
	//getClaimsPageData($scope, $http, new Date());

	$scope.$watchGroup(['context.selectedDate', 'context.currentLocation'], function(newValues, oldValues, scope) {
		if (newValues && newValues.length > 0) {
		  getClaimsPageData($scope, $http, aggregator, regionData, $scope.context.selectedDate.toISOString(), $scope.context.currentLocation);
		}
	});
}

function getClaimsPageData($scope, $http, aggregator, regionData, date, location) {
	regionData.getGeoIDFromLocation(location).then(function(geo) {
		if (geo.region) {
			aggregator.getSubregionsClaimsData(geo.country, geo.region, date).then(function(data) {
				$scope.claimsdetails = data.claimsdetails;
				$scope.claimsstats = data.claimsstats;
			}, function(err) {
				console.log(err);
			});
		} else {
			aggregator.getRegionsClaimsData(geo.country, date).then(function(data) {
				$scope.claimsdetails = data.claimsdetails;
				$scope.claimsstats = data.claimsstats;
			}, function(err) {
				console.log(err);
...
// directives
app.directive('claimsPage', [claimsPageDirective]);
