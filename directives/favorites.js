...
function favoritesController($scope, $http){
	userFavorites($scope, $http);
...
function userFavorites($scope, $http) {
	// need to take that from the session (and not currentUser);
	var user = "user";
	$http.get('/data/favorites'). success(function(data) {
		$scope.favorites = data.slice().reverse();
	});
}
...
// directives
app.directive('favorites', [favoritesDirective]);
