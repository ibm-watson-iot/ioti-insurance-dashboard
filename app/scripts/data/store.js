(function() {
  ...
  adapters = ['ApplicationAdapter', 'UserAdapter', 'ClaimAdapter'];
  deps = models.concat(adapters);
  deps.push('createRecordClass', '$rootScope');
  deps.push(function() {
    var CustomDataStore, adaptersMap, args, modelDefs, adaptersDefs, createRecordClass, $rootScope;
    args = Array.prototype.slice.call(arguments);
    modelDefs = args.slice(0, models.length);
    adaptersDefs = args.slice(models.length);
    createRecordClass = args.slice(-2)[0];
    $rootScope = args.slice(-1)[0];
