...
util.validateUser = function(req, res, accessLevel, callback){
	...
	var username = req.session.user || req.body.username;
	var password = req.session.pass || req.body.password;
	
	db.checkUserCredentialsFull( username, password, function(err, doc) {
		if (err) {
			callback(err,null);
		} else {
