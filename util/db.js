...
var cloudant = credentials.getCredentialsByServiceName('cloudantNoSQLDB');
...
if (cloudant) {
    ...
    db.checkUserCredentialsFull = function(username, password, callback) {
        var query = {
            "selector": {
                "username": username,
                "password": db.hashPasswordWithSalt(password, username)
            }
        };

        var mydb = db.getDB("users", true);

        mydb.find(query, function(err, doc) {
            if (err) {
                console.log(err);
                callback(err, null);
            } else {
                if (doc.docs && doc.docs.length == 1) {
                    callback(null, doc.docs[0]);
                } else {
                    callback("user/pass not found", null);
                }
            }
        });
    };
    ...
    db.getUserHazardEvents = function(username, includeDocs, callback) {
        var incDocs = includeDocs || true;
        var mydb = eventsdb;

        mydb.view('userhazards', 'userview', {
            keys: [username],
    ...
    db.getUserMessages = function(username, includeDocs, callback) {
        var incDocs = includeDocs || true;
        var mydb = messagesdb;
        
        mydb.view('usermessages', 'userview', {
            keys: [username],
            include_docs: incDocs
        }, function(err, doc) {
            if (err) {
    ...
    db.getAllShields = function(callback) {
        var mydb = shieldsdb;
        mydb.list({
            include_docs: true
        }, function(err, doc) {
    ...
    db.getUserShieldAssociations = function(username, callback) {
        var mydb = shieldassociationsdb;
        mydb.view('shields', 'userview', {
            keys: [username],
            include_docs: true
        }, function(err, doc) {
            if (err) {
    ...
    db.addFavoritesByUsername = function(username, favorite, callback) {
        ...
        mydb.get(username, function(err, doc) {
            if (err) {
                ...
                if (doc.favorites.indexOf(favorite) === -1) {
                    doc.favorites.push(favorite);

                    mydb.insert(doc, username, function(err, response) {
                        if (err) {
                            callback(err, null);
                        } else {
                            callback(null, doc.favorites);
    ...
    db.getUserDevices = function(username, includeDocs, callback) {
        var incDocs = includeDocs || true;
        var mydb = devicesdb;

        mydb.view('userdevices', 'usersview', {
            keys: [username],
            include_docs: incDocs
        }, function(err, doc) {
            if (err) {
