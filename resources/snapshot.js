...
var swaggererr = swagger.errors;
var paramTypes = swagger.paramTypes;

var db = require("../util/db.js");
var util = require("../util/util.js");

var aggregation_db = db.getDB("aggregation");

function getSnapshotCloudant(country, region, subregion, beforeDate, subLevel, callback) {
    // TODO: sublevel calculations not effective. Lucene might be better choice
    var params = {
        descending: true,
        limit: subLevel ? 200 : 1, // Only want the latest one for non-sublevel
        include_docs: true,
        startkey: [],
        endkey: []
    };
    var view = "world";
    if (country) {
        params.startkey.push(country);
        params.endkey.push(country);
        view = subLevel ? "regions" : "country";
    }
    if (region) {
        params.startkey.push(region);
        params.endkey.push(region);
        view = subLevel ? "subregions" : "region";
    }
    if (subregion) {
        params.startkey.push(subregion);
        params.endkey.push(subregion);
        view = "subregion";
    }
    if (beforeDate) {
        params.startkey.push(beforeDate);
    } else {
        params.startkey.push("\u0fff");
    }
    params.endkey.push(null);
    params.endkey.push({});
    aggregation_db.view("timestamp", view, params, function(err, result) {
        if (err) {
            callback(err, result);
        } else {
            if (subLevel) {
                ...
                callback(err, result.rows[0] ? result.rows[0].doc : null);
            }
        }
    });
}

var getSnapshot = function(country, region, subregion, beforeDate, subLevel, res) {
    getSnapshotCloudant(country, region, subregion, beforeDate, subLevel, function(err, result) {
        if (err) {
            swaggererr.notFound('snapshot', res);
        } else {
...
module.exports = snapshot = {
    getSnapshotSubregion: {
        'spec': {
            description: "Get the snapshot for subregion",
            path: "/snapshot/{country}/{region}/{subregion}",
            method: "GET",
            summary: "Get the snapshot for subregion",
            notes: "Get the snapshot for subregion.",
            nickname: "getSnapshotSubregion",
            produces: ["application/json"],
            parameters: [paramTypes.path("country", "Country", "string"),
                paramTypes.path("region", "Region (for example US state code)", "string"),
                paramTypes.path("subregion", "Subregion (for example US county code)", "string"),
                paramTypes.query("beforeDate", "Latest snapshot before date/time", "string", false)
            ],
            type: "Snapshot",
            responseMessages: [swaggererr.notFound('snapshot')]
        },
        'action': function(req, res) {
            ...
            util.validateUser(req, res, 13, function(er, validated) {
                if (!validated)
                	swaggererr.forbidden(res);
                else {
                    getSnapshot(req.params.country, req.params.region, req.params.subregion, req.query.beforeDate, false, res);
                }
            });
    ...
    getSnapshotRegion: {
        'spec': {
            description: "Get the snapshot for region",
            path: "/snapshot/{country}/{region}",
            method: "GET",
            summary: "Get the snapshot for region",
            notes: "Get the snapshot for region.",
            nickname: "getSnapshotRegion",
            produces: ["application/json"],
            parameters: [paramTypes.path("country", "Country", "string"),
                paramTypes.path("region", "Region (for example US state code)", "string"),
                paramTypes.query("beforeDate", "Latest snapshot before date/time", "string", false),
                paramTypes.query("subLevel", "Returns a list of subregions of the region instead of the region", "string", false)
            ],
            type: "Snapshot",
            responseMessages: [swaggererr.notFound('snapshot')]
        },
        'action': function(req, res) {
            ...
            util.validateUser(req, res, 13, function(er, validated) {
                if (!validated)
                    ...
                    getSnapshot(req.params.country, req.params.region, null, req.query.beforeDate, req.query.subLevel === "true", res);
                }
            });
        }
    },
    ...
    getSnapshotCountry: {
        'spec': {
            description: "Get the snapshot for country",
            path: "/snapshot/{country}",
            method: "GET",
            summary: "Get the snapshot for country",
            notes: "Get the snapshot for country.",
            nickname: "getSnapshotCountry",
            produces: ["application/json"],
            parameters: [paramTypes.path("country", "Country", "string"),
                paramTypes.query("beforeDate", "Latest snapshot before date/time", "string", false),
                paramTypes.query("subLevel", "Returns a list of regions of the country instead of the country", "string", false)
            ],
            type: "Snapshot",
            responseMessages: [swaggererr.notFound('snapshot')]
        },
        'action': function(req, res) {
            ...
            util.validateUser(req, res, 13, function(er, validated) {
                if (!validated)
                    ...
                    getSnapshot(req.params.country, null, null, req.query.beforeDate, req.query.subLevel === "true", res);
                }
            });
        }
    }
